      ...
      <Card className="border-l-2 border-l-primary">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {["Loss", "Learning Rate"].map((label) => (
              <div key={label} className="rounded-md bg-muted px-3 py-3 border border-border">
                <p className="text-xs text-muted-foreground">{label}</p>
                <p className="mt-1 font-mono text-2xl font-semibold tabular-nums">--</p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-4 gap-2">
            {["Tokens/sec", "GPU Mem", "Epoch", "ETA"].map((label) => (
          ...
          <p className="text-center text-xs text-muted-foreground">
            Configure corpus and tokenizer to enable.
          </p>
        </CardContent>
      </Card>
