        ...
        <CardContent className="space-y-4">
          {status === "idle" && lossHistory.length === 0 ? (
            <div className="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-border text-center">
              <p className="text-sm text-muted-foreground">No live training process</p>
              <p className="mt-1 text-xs text-muted-foreground">Metrics appear once training starts.</p>
            </div>
          ) : (
            <>
          ...
          {isRunning ? (
            <Button className="w-full h-10 gap-2 text-sm" variant="destructive" onClick={stop}>
              <Square className="h-4 w-4" />
              Stop Training
            </Button>
          ) : (
            <Button className="w-full h-10 gap-2 text-sm" variant="secondary" onClick={start}>
              <Play className="h-4 w-4" />
              {status === "done" ? "Replay Demo" : "Start Training"}
            </Button>
          )}
        </CardContent>
      </Card>
